...
"""
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="aymen666FF", title_id=title_id)
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="aymen666FF", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
def get_player_add(uid):
    try:
        url = f"https://hamza-add.vercel.app/adding_friend?uid=GUEST_UID&password=YOUR_GUEST_UID&friend_uid={uid}"
        res = requests.get(url)
        data = res.json()
...
def get_player_add_1(uid):
    try:
        url = f"https://hamza-add.vercel.app/adding_friend?uid=4379818952&password=FLASH_FF_KING_ZL4Y&friend_uid={uid}"
        res = requests.get(url)
...
def get_player_add_2(uid):
    try:
        url = f"https://hamza-add.vercel.app/adding_friend?uid=4379808011&password=FLASH_FF_KING_M6QE&friend_uid={uid}"
        res = requests.get(url)
        return res.json().get('message', 'No message')
    except Exception as e: return f"Error: {e}"
...
def get_player_remove(uid):
    try:
        url = f"https://hamza-add.vercel.app/remove_friend?uid=GUEST_UID&password=YOUR_GUEST_UID&friend_uid={uid}"
        res = requests.get(url)
        data = res.json()
            # add is inside socialInfo -> signature
        action = data.get('action', 'Unknown')
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]ARAFAT [00FF00]!!
"""
...
[FFFFFF]Example: /bundle midnight"""
                        ...
                        if inPuTMsG.startswith(("/5")):
                            ...
                            # SUCCESS MESSAGE
                            success_message = f"[B][C][00FF00]✅ تم بنجاح! تم إرسال دعوة المجموعة بنجاح إلى {uid}!\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
...
[FFFFFF]          ⚡ PICOS S 1 X ❤️  
[FFFFFF]                   thank for support 
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]PICOS S 1 X    
[FFD700]✨ If anyone wants to buy a GUILD BOT, message on Telegram, telegram username @Picos0 ❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> PICOS S 1 X ❄️  ⚡
"""
                        ...
                        if inPuTMsG.strip().startswith('/multijoin'):
                            ...
                            if len(parts) < 2:
                                ...
                                if not target_uid.isdigit():
                                    error_msg = f"[B][C][FF0000]❌ يرجى كتابة معرّف لاعب صالح!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
        
                                initial_msg = f"[B][C][00FF00]🚀 جارٍ بدء هجوم الانضمام المتعدد على {target_uid}...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    if success_count > 0:
                                        result_msg = f"""
[B][C][00FF00]✅ تم إكمال هجوم الانضمام المتعدد!
...
💡 تحقّق من لعبتك بحثًا عن طلبات الانضمام!
"""
                                    else:
                                        result_msg = f"[B][C][FF0000]❌ فشلت جميع طلبات الانضمام! تحقّق من اتصال البوت.\n"
                        ...
                        if inPuTMsG.strip().startswith('/fastmultijoin'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /fastmultijoin (uid)\nExample: /fastmultijoin 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                accounts_data = load_accounts()
                                if not accounts_data:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! لم يتم العثور على أي حسابات!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
                                
                                initial_msg = f"[B][C][00FF00]⚡ إرسال انضمام سريع ومتكرر بعدة حسابات!\n🎯 Target: {target_uid}\n👥 Accounts: {len(accounts_data)}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]✅ تم إكمال الانضمام السريع المتعدد!\n🎯 Target: {target_uid}\n✅ Successful: {join_count}/{len(accounts_data)}\n⚡ Speed: Ultra fast\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/reject'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /reject (target_uid)\nExample: /reject 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                # Send start message
                                start_msg = f"[B][C][1E90FF]🌀 تم بدء الإرسال المتكرر للرفض: {target_uid}\n🌀 Packets: 150 each type\n🌀 Interval: 0.2 seconds\n"
                        ...
                        if inPuTMsG.strip() == '/reject_stop':
                            if reject_spam_task and not reject_spam_task.done():
                                reject_spam_running = False
                                reject_spam_task.cancel()
                                stop_msg = f"[B][C][00FF00]✅ تم إيقاف الإرسال المتكرر للرفض بنجاح!\n"
                                await safe_send_message(response.Data.chat_type, stop_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ لا يوجد إرسال رفض متكرر (Reject Spam) نشط لإيقافه!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /room (uid)\nExample: /room 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                room_id = parts[2]
                                ...
                                if not target_uid.isdigit():
                                    error_msg = f"[B][C][FF0000]❌ خطأ! يُرجى كتابة معرّف لاعب صالح!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
        
                                # Send initial message
                                initial_msg = f"[B][C][00FF00]🔍 جارٍ العمل على الإرسال المتكرر داخل الغرفة من أجل{target_uid}...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    room_msg = f"[B][C][00FF00]🎯 تم اكتشاف لاعب داخل الغرفة {room_id}\n"
                                    ...
                                    start_msg = f"[B][C][00FF00]🚀 جارٍ بدء الإرسال المتكرر: {spam_count} packets to room {room_id}\n"
                                    await safe_send_message(response.Data.chat_type, start_msg, uid, chat_id, key, iv)
                                    ...
                                    success_msg = f"[B][C][00FF00]✅ تم إكمال الإرسال المتكرر داخل الغرفة!\n🎯 Target: {target_uid}\n📦 Packets: {spam_count}\n🏠 Room: {room_id}\n⚡ Speed: Ultra fast\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    print(f"Room spam completed for UID: {target_uid}")
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ خطأ في الإرسال المتكرر داخل الغرفة: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/spam'):
                            ...
                            if len(parts) < 2:
                                ...
                                initial_msg = f"[B][C][1E90FF]🌀 تم استلام الطلب! جارٍ التحضير للإرسال المتكرر {target_uid} مع جميع الشارات...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                success_msg = f"[B][C][00FF00]✅ تم الإرسال بنجاح {total_requests} طلبات الانضمام!\n🎯 Target: {target_uid}\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/joinroom'):
                            ...
                            if len(parts) < 3:
                                ...
                                initial_msg = f"[B][C][00FF00]🚀جارٍ الانضمام إلى الغرفة المخصّصة...\n🏠 Room: {room_id}\n🔑 Password: {room_password}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]✅ تم الانضمام إلى الغرفة المخصّصة {room_id}!\n🤖البوت موجود الآن في دردشة الغرفة!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! Failed to join room: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/createroom'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /createroom (room_name) (password) [players=4]\nExample: /createroom BOTROOM 0000 4\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_name = parts[1]
                                room_password = parts[2]
                                ...
                                initial_msg = f"[B][C][00FF00]🏠 جارٍ إنشاء غرفة مخصّصة...\n📛 Name: {room_name}\n🔑 Password: {room_password}\n👥 Max Players: {max_players}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]✅ تم إنشاء غرفة مخصّصة!\n🏠 Room: {room_name}\n🔑 Password: {room_password}\n👥 Max: {max_players}\n🤖 Bot is now hosting!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.startswith('/join'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /join (team_code)\nExample: /join ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                sender_uid = response.Data.uid  # Get the UID of person who sent the command
        
                                initial_message = f"[B][C]{get_random_color()}\nجارٍ الانضمام إلى الفريق باستخدام الرمز: {CodE}...\n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_message = f"[B][C][00FF00]✅ تم بنجاح! تم الانضمام إلى الفريق: {CodE}!\n💍 تم تفعيل تعبير الحلقتين المزدوجتين!\n🤖 Bot + You = 9lawe\n"
                                    await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                                    ...
                                    try:
                                        ...
                                        if ghost_packet:
                                            ...
                                            success_message = f"[B][C][00FF00]✅ تم بنجاح! تم الانضمام الشبحِي إلى الفريق: {CodE}!\n💍 تم تفعيل تعبير الحلقتين المزدوجتين!\n🤖 Bot + You = 9lawe\n"
                                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                                        else:
                                            error_msg = f"[B][C][FF0000]❌ خطأ! فشل إنشاء حزمة الانضمام الشبحِ.\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/ghost'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /ghost (team_code)\nExample: /ghost ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nجارٍ الانضمام الشبحِ إلى الفريق باستخدام الرمز...: {CodE}...\n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    if ghost_packet:
                                        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', ghost_packet)
                                        success_message = f"[B][C][00FF00]✅ تم بنجاح! تم الانضمام الشبحِ إلى الفريق باستخدام الرمز: {CodE}!\n"
                                        await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                                    else:
                                        error_msg = f"[B][C][FF0000]❌ خطأ! فشل إنشاء حزمة الانضمام الشبحِ\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    ...
                                    error_msg = f"[B][C][FF0000]❌ خطأ! فشل الانضمام الشبحِ: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lag '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /lag (team_code)\nExample: /lag ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                ...
                                # SUCCESS MESSAGE
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم بدء هجوم التأخير (Lag)!\nTeam: {team_code}\nAction: انضمام/مغادرة سريعة\nالسرعة: فائقة للغاية (milliseconds)\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop lag':
                            if lag_task and not lag_task.done():
                                lag_running = False
                                lag_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم إيقاف هجوم التأخير (Lag) بنجاح!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ خطأ! لا يوجد هجوم تأخير (Lag) نشط لإيقافه!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

                        if inPuTMsG.startswith('/exit'):
                            # Process /exit command in any chat type
                            initial_message = f"[B][C]{get_random_color()}\nجارٍ مغادرة الفريق الحالي....\n"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            ...
                            # SUCCESS MESSAGE
                            success_message = f"[B][C][00FF00]✅ تم بنجاح! غادرت الفريق بنجاح!\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/start'):
                            ...
                            # SUCCESS MESSAGE
                            success_message = f"[B][C][00FF00]✅ تم بنجاح! تم إرسال أمر بدء المباراة!\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/e'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /e (uid) (emote_id)\nExample: /e 123456789 909000001\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
                                
                            initial_message = f'[B][C]{get_random_color()}\nجارٍ إرسال الرقصة إلى الهدف...\n'
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            ...
                            if not s:
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_msg = f"[B][C][00FF00]✅ تم بنجاح! Emote {idT} sent to {len(target_uids)} player(s)!\nTargets: {', '.join(map(str, target_uids))}\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ خطأ أثناء إرسال الرقصة: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Invalid UID format. Usage: /e (uid) (emote_id)\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /lvup (team_code)\nExample: /lvup 123456\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                ...
                                if not team_code.isdigit():
                                    error_msg = f"[B][C][FF0000]❌ خطأ! يجب أن يتكوّن رمز الفريق من أرقام فقط!\nExample: /lvup 123456\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/evos'):
                            ...
                            if len(uids) == 1:
                                ...
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! بدأت دورة الرقصات المتطورة!\n🎯 Targets: انت + {len(uids)-1} اللاعبون الآخرون\n🎭 Emotes: All 18 evolution emotes\n⏰ Delay: 5 seconds between emotes\n🔄 Cycle: Continuous loop until /sevos\n"
                        ...
                        if inPuTMsG.strip() == '/sevos':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم إيقاف دورة الرقصات متطورة بنجاح!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                print("Evolution emote cycle stopped by command")
                            else:
                                error_msg = f"[B][C][FF0000]❌ خطأ! لا توجد دورة رقصات متطورة نشطة لإيقافها!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/fast'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /fast uid1 [uid2] [uid3] [uid4] [1-410]\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not emote_id:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح! Usage: /fast uid1 [uid2] [uid3] [uid4] [1-410]\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_msg = f"[B][C][00FF00]✅ تم بنجاح! بدأ سبام الإيموجي السريع!\nTargets: {len(uids)} players\nEmote: {emote_id}\nSpam count: 25 times\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/p'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /p (uid) [1-410] (times)\nExample: /p 123456789 [1-410] 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                try:
                                    target_uid = parts[1]
                                    ...
                                    if times <= 0:
                                        error_msg = f"[B][C][FF0000]❌ خطأ! يجب أن يكون عدد المرات أكبر من 0!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    elif times > 100:
                                        error_msg = f"[B][C][FF0000]❌ خطأ! الحد الأقصى المسموح به هو 100 مرة لأسباب تتعلق بالسلامة!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                        ...
                                        # SUCCESS MESSAGE
                                        success_msg = f"[B][C][00FF00]✅ تم بنجاح! بدأ تكرار الرقصة المخصص!\nTarget: {target_uid}\nEmote: {emote_id}\nTimes: {times}\n"
                                        await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                        
                                except ValueError:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق الرقم غير صالح! Usage: /p (uid) [1-410] (times)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                except Exception as e:
                        ...
                        if inPuTMsG.strip().startswith('/spm_inv'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /spm_inv (uid)\nExample: /spm_inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip() == '/stop spm_inv':
                            if spam_request_task and not spam_request_task.done():
                                spam_request_running = False
                                spam_request_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم إيقاف طلب السبام بنجاح!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                        ...
                        if inPuTMsG.strip().startswith('/play '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /play uid1 [uid2] [uid3] [uid4] number(1-410)\nExample: /play 123456789 1\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح! Usage: /play uid1 [uid2] [uid3] [uid4] number(1-410)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        if number_int not in ALL_EMOTE:
                                            error_msg = f"[B][C][FF0000]❌ خطأ! يجب أن يكون الرقم بين 1 و410 فقط\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            initial_message = f"[B][C]{get_random_color()}\nجارٍ إرسال الرقصة {number_int}...\n"
                                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                            
                                            success, result_msg = await play_emote_spam(uids, number_int, key, iv, region)
                                            
                                            if success:
                                                success_msg = f"[B][C][00FF00]✅ تم بنجاح!{result_msg}\n"
                                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                            else:
                                        ...
                                        error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق رقم غير صالح! استخدم الأرقام من 1 إلى 410 فقط.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/100 '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ خطأ! Usage: /100 uid1 [uid2] [uid3] [uid4]\nExample: /100 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                # Parse uids and number
                                uids = []
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح.! Usage: /100 uid1 [uid2] [uid3] [uid4]\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = 268
                        ...
                        if inPuTMsG.strip().startswith('/evo '):
                            ...
                            if len(parts) < 2:
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح! Usage: /evo uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        if number_int not in EMOTE_MAP:
                                            error_msg = f"[B][C][FF0000]❌ خطأ! يجب أن يكون الرقم بين 1 و21 فقط!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            initial_message = f"[B][C]{get_random_color()}\جار تنفيذ الرقصة{number_int}...\n"
                                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                            
                                            success, result_msg = await evo_emote_spam(uids, number_int, key, iv, region)
                                            
                                            if success:
                                                success_msg = f"[B][C][00FF00]✅ ناجح! {result_msg}\n"
                                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                            else:
                                                error_msg = f"[B][C][FF0000]❌ خطأ! {result_msg}\n"
                                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        ...
                                        error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق رقم غير صالح! استخدم الأرقام من 1 إلى 21 فقط.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/evo_fast '):
                            ...
                            if len(parts) < 2:
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح! Usage: /evo_fast uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        if number_int not in EMOTE_MAP:
                                            ...
                                            success_msg = f"[B][C][00FF00]✅ تم بنجاح! بدأ سبام رقصات المتطورة السريع!\nTargets: {len(uids)} players\nEmote: {number_int} (ID: {emote_id})\nSpam count: 25 times\nInterval: 0.1 seconds\n"
                                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/evo_c '):
                            ...
                            if len(parts) < 3:
                                ...
                                if not uids or not number or not time_val:
                                    error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق غير صالح! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-21) time(1-100)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        ...
                                        if number_int not in EMOTE_MAP:
                                            ...
                                            error_msg = f"[B][C][FF0000]❌ خطأ! يجب أن يكون الوقت بين 1 و100 فقط!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            ...
                                            # SUCCESS MESSAGE
                                            emote_id = EMOTE_MAP[number_int]
                                            success_msg = f"[B][C][00FF00]✅ تم بنجاح! بدأ سبام الرقصات المتطورة المخصص!\nTargets: {len(uids)} players\nEmote: {number_int} (ID: {emote_id})\nRepeat: {time_int} times\nInterval: 0.1 seconds\n"
                                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                            
                                    except ValueError:
                                        error_msg = f"[B][C][FF0000]❌ خطأ! تنسيق الرقم/الوقت غير صالح! استخدم أرقامًا فقط.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_fast':
                            if evo_fast_spam_task and not evo_fast_spam_task.done():
                                evo_fast_spam_running = False
                                evo_fast_spam_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم إيقاف سبام التطور السريع بنجاح\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ خطأ! لا يوجد سبام تطور سريع نشط لإيقافه!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                evo_custom_spam_running = False
                                evo_custom_spam_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ تم بنجاح! تم إيقاف سبام التطور المخصص بنجاح\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ ERROR! لا يوجد تطوّر نشط أو تخصيص لمنع الرسائل المزعجة!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        # status message 
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # ───── الأوامر المتقدمة ─────
                            group_commands = """[C][B][FFD700]═══⚡ الأوامر المتقدمة ⚡═══[00FF00][B]
├─ [00FF00]⚡إرسال لايكات ✅️✅️
│  └─ [00AAFF]/likes [uid]
├─ [00FF00]فتح سكواد 4
│  └─ [00AAFF]/3
├─ [00FF00]فتح سكواد 5 
...
├─ [00FF00]فتح سكواد 6
│  └─ [00AAFF]/6
├─ [00FF00]فتح سكواد 5 لصديق
│  └─ [00AAFF]/inv [uid]
├─ [00FF00]انضمام البوت إلى الفريق
│  └─ [00AAFF]/join [team_code]
└─ [00FF00]إخراج البوت من الفريق
   └─ [00AAFF]/exit
[00FF00]━━━━━━━━━━━━[00AAFF]"""
                            ...
                            # ───── الأوامر المتقدمة ─────
                            advanced_commands = """[C][B][800080]═══⚡ الأوامر المتقدمة ⚡═══[00FF00][B]
├─ [00FF00]تشغيل بوت رفع المستوى
│  └─ [00AAFF]/lw [team code]
├─ [00FF00]إيقاف بوت رفع المستوى
│  └─ [00AAFF]/stop
├─ [00FF00]ارتداء الحزمة النادرة
│  └─ [00AAFF]/bundle [code]
├─ [00FF00]لاڨ الفريق
│  └─ [00AAFF]/lag [code]
├─ [00FF00]ايقاف لاڨ 
│  └─ [00AAFF]/stop lag
└─ [00FF00]رفض الرسائل المزعجة (سبام)
   └─ [00AAFF]/reject [uid]
[00FF00]━━━━━━━━━━━━[00AAFF]"""
                            ...
                            # ───── قائمة رقصات ─────
                            emote_commands = """[C][B][32CD32]═══⚡ أوامر رقصات ⚡═══[7CFC00][B]
├─ [7CFC00]جميع رقصات فري فاير
│  └─ [32CD32]/play [uid] [1-410]
├─ [7CFC00]رقصات سريعة (25 مرة)
│  └─ [32CD32]/fast [uid] [1-410]
├─ [7CFC00]رقصة مخصصة (عدد X مرات)
│  └─ [32CD32]/p (uid) [1-410] (times)
├─ [7CFC00]رقصة ليفل 100
│  └─ [32CD32]/play [uid] 268
├─ [7CFC00]قائمة رقصات
│  └─ [32CD32]/emote
└─ [7CFC00]رقصات مخصصة (باستخدام المعرف ID)
   └─ [32CD32]/e [uid] [id]
[7CFC00]━━━━━━━━━━━━[32CD32]"""
                            await safe_send_message(response.Data.chat_type, emote_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── أوامر رقصات الأسلحة متطورة ─────
                            evo_commands = """[C][B][00AAFF]═══⚡ رقصات الأسلحة متطورة ⚡═══[00FF00][B]
├─ [00FF00]جميع رقصات evo
│  └─ [00AAFF]/evo [uid] [1-21]
├─ [00FF00]رقصات Evo سريع (25 مرة)
│  └─ [00AAFF]/evo_fast [uid] [1-21]
├─ [00FF00]رقصات Evo مخصص (عدد X مرات)
│  └─ [00AAFF]/evo_c [uid] [1-21] [x]
├─ [00FF00]تنفيذ جميع رقصات evo
│  └─ [00AAFF]/evos [uid]
└─ [00FF00]ايقاف دورة رقصات evo
   └─ [00AAFF]/sevos
[00FF00]━━━━━━━━━━━━[00AAFF]"""
                            ...
                            ai_commands = """[C][B][00AAFF]═══⚡ أدوات وأوامر ترفيهية ⚡═══[00FF00][B]
├─ [00FF00]الحصول على نبذة اللاعب باستخدام UID
│  └─ [00AAFF]/bio [uid]
├─ [00FF00]جلب معلومات مستخدم إنستغرام
│  └─ [00AAFF]/ig [username]
├─ [00FF00]إرسال رسالة سبام مخصصة
│  └─ [00AAFF]/ms <text>
├─ [00FF00]اسأل الذكاء الاصطناعي أي شيء
│  └─ [00AAFF]/ai [question]
├─ [00FF00]معلومات المشرف
│  └─ [00AAFF]/admin
└─ [00FF00]التحقق من حالة البوت
   └─ [00AAFF]/status
[00FF00]━━━━━━━━━━━━[00AAFF]"""
                            ...
                            # ───── أوامر الشارات ─────
                            badge_commands = """[C][B][00AAFF]═══⚡ طلبات الانضمام إلى الشارات ⚡═══[00FF00][B]
├─ [00FF00]قبول طلبات الانضمام لشارة كرافتلاند
│  └─ [00AAFF]/s1 [uid]
├─ [00FF00]ادخال البوت داخل روم
│  └─ [00AAFF]/joinroom [uid]
├─ [00FF00]سبام داخل روم
│  └─ [00AAFF]/room [uid]
├─ [00FF00]قبول طلبات الانضمام للشارة الصغيرة V
│  └─ [00AAFF]/s4 [uid]
├─ [00FF00]سبام مخفف
│  └─ [00AAFF]/s5 [uid]
└─ [00FF00]سبام طلبات الإنضمام للفريق
   └─ [00AAFF]/spam [uid]
[00FF00]━━━━━━━━━━━━[00AAFF]"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # ───── أوامر المعلومات ─────
                            info_commands = """[C][B][00AAFF]═══⚡ أدوات وأوامر ترفيهية ⚡═══[00FF00][B]
├─ [00FF00]الحصول على المعلومات الأساسية للاعب
│  └─ [00AAFF]/info [uid]
├─ [00FF00]التحقق من حالة حظر الحساب
│  └─ [00AAFF]/check [uid]
├─ [00FF00]إضافة البوت إلى قائمة الأصدقاء
│  └─ [00AAFF]/add [uid]
├─ [00FF00]إزالة من قائمة الأصدقاء
│  └─ [00AAFF]/remove [uid]
├─ [00FF00]إرسال طلبات صداقة بشكل متكرر 
│  └─ [00AAFF]/spam_inv [uid]
└─ [00FF00]ابحث عن أي صديق
   └─ [00AAFF]/gali [name]
[00FF00]━━━━━━━━━━━━[00AAFF]"""
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("Initializing PICOS STAR Bot...")
    print("┌────────────────────────────────────┐")
    print("│ █████████████░░░░░░░░░░░░░░░░░░ │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
    ...
    os.system('clear')
    print(render('PICOS', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 PICOS BOT - ONLINE")
    print(f"🔹 UID: {TarGeT}")
    print(f"🔹 Name: {acc_name}")
